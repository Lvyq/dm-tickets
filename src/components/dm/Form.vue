		...
		<a-form :model="form" :style="{ width: '800px' }" @submit="handleSubmit">
			<a-form-item field="cookie" label="cookie" required>
				...
				<div style="width: 100%;">
					...
					<a-textarea style="width: 100%;" v-model="form.cookie" placeholder="请输入 cookie" allow-clear />
				</div>
			</a-form-item>

			<a-form-item field="url" label="演出链接">
				<div style="width: 100%;">
					<a-input style="width: 100%;" v-model="form.url" placeholder="请输入商品详情页url" />
				</div>
			</a-form-item>

			<a-form-item field="itemId" label="演出ID" required>
				<template #extra>
					<div>进入商品页面，页面路径上的 itemId</div>
				</template>
				<div style="width: 100%;">
					<a-input style="width: 100%;" v-model="form.itemId" placeholder="请输入itemId..." />
				</div>
			</a-form-item>
				...
				<a-checkbox-group v-model="form.selectVisitUserList" @change="userChange">
					<a-checkbox v-for="item in visitUserList" :key="item.maskedIdentityNo"
						...
						<template #checkbox="{ checked }">
							<a-space style="margin:2px;" align="start" class="custom-checkbox-card" :class="{
			...
			<a-button style="width: 30%;margin: 0 auto;margin-bottom: 20px;" size="mini" @click="setLoading"
				type="primary" :loading="isLoading">获取观演人员</a-button>
			...
			<a-form-item>
				<a-button type="primary" html-type="submit" style="width: 100%;">保存设置</a-button>
			</a-form-item>
		</a-form>
	</section>
</template>
