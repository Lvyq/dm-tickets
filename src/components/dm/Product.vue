<script setup lang="js">
	...
	async function getProductInfo() {
		const data =
			`{"itemId":"${form.value.itemId}","bizCode":"ali.china.damai","scenario":"itemsku","exParams":"{\\"dataType\\":4,\\"dataId\\":\\"\\",\\"privilegeActId\\":\\"\\"}","dmChannel":"damai@damaih5_h5"}`;
	...
	async function getSkuInfo(item) {
		...
		activeItemId.value = performId;
		skuList.value = null;
		activeSku.value = null;
		
		
		const data =
			`{"itemId":"${form.value.itemId}","bizCode":"ali.china.damai","scenario":"itemsku","exParams":"{\\"dataType\\":2,\\"dataId\\":\\"${performId}\\",\\"privilegeActId\\":\\"\\"}","dmChannel":"damai@damaih5_h5"}`;
		const [t, sign] = getSign(data, form.value.token);
			...
			if (Array.isArray(parseData.ret) && parseData.ret.length) {
				...
				if (isSuccess(message)) {
					const result = parseData.data ?
						...
						"";
					if (result) {
						skuList.value = result.perform.skuList;
	...
	async function getOrderDetail(item) {
		const data =
			`{"buyNow":true,"exParams":"{\\"channel\\":\\"damai_app\\",\\"damai\\":\\"1\\",\\"umpChannel\\":\\"100031004\\",\\"subChannel\\":\\"damai@weixin_gzh\\",\\"atomSplit\\":1,\\"serviceVersion\\":\\"2.0.0\\",\\"customerType\\":\\"default\\"}","buyParam":"${item.itemId}_${form.value.num}_${item.skuId}","dmChannel":"damai@damaih5_h5"}`;
			...
			if (Array.isArray(parseData.ret) && parseData.ret.length) {
				...
				if (isSuccess(message)) {
					...
					if (msg.includes(VALIDATE)) {
						const newMsg = '检测到账号已被拦截，请重新生成 cookie，即将退出抢票'
						Message.error(newMsg)
		...
		<div v-if="productInfo">
			<div class="info-wrap" v-if="productInfo.staticData">
				<div class="left">
					...
					<a-button v-if="isPreSell" style="margin-bottom: 10px;width: 100%;" type="primary" status="success"
						@click="rob" :loading="isRob" :disabled="!activeSku">开始抢票</a-button>
					<a-button v-else style="margin-bottom: 10px;width: 100%;" type="primary" status="normal"
						@click="buy" :disabled="!activeSku">立即购票</a-button>
				</div>
				...
				<div class="right">
					...
					<div class="ticket-list">
						<div v-for="item in productInfo.item.performBases" class="ticket-item" 
							@click="getSkuInfo(item)">
							<div class="ticket-item-head" :class="{
					            'active-sku':
					                activeItemId &&
					                activeItemId === item.performs[0].performId,
					        }">
								{{ item.name }}

								<a-tag v-if="item.performBaseTagDesc" :color="
					...
					<h4>票档</h4>
					<div class="sku" style="width:100%;display: flex;flex-direction: row;">
						
						<div class="sku-item" :class="{
					            'active-sku':
					                activeSku &&
					                activeSku.skuId === sku.skuId,
					        }" v-for="sku in skuList" @click="saveSku(sku)">
